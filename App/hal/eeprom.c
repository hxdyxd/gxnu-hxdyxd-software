...
int eeprom_write(uint16_t addr, uint8_t *buf, int len)
{
    for(int i=0; i<len; i+=8) {
        ...
        if(HAL_I2C_Mem_Write(&hi2c1, 0xa0, addr, I2C_MEMADD_SIZE_8BIT, buf + i, wsize, 20) != HAL_OK) {
...
{
    return (HAL_I2C_Mem_Read(&hi2c1, 0xa1, addr, I2C_MEMADD_SIZE_8BIT, buf, len, 20) == HAL_OK);
}
